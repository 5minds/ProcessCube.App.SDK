...
export class FlowNode {
  ...
  get IsCallActivity(): boolean {
    return this.type === 'bpmn:CallActivity';
  }

  get IsUserTask(): boolean {
    return this.type === 'bpmn:UserTask';
  }

  get IsManualTask(): boolean {
    return this.type === 'bpmn:ManualTask';
  }
...
export default class BpmnViewerOverlayCreator {
  ...
  public createOverlaysFlowNodeInstances(
    processInstanceState: string,
    ...
    retryAction: (processInstanceId: string, flowNodeInstanceId?: string, newToken?: string) => void | undefined,
    ...
    for (const executedFlowNode of executedFlowNodes) {
      ...
      root?.render(
        React.createElement(FlowNodeOverlay, {
          flowNode: executedFlowNode,
          width: flowNodeShape.width,
          height: flowNodeShape.height,
          retryAction: retryAction,
          gotoProcessAction: gotoProcessAction,
          gotoManualOrUserTaskAction: gotoManualOrUserTaskAction,
        }),
      );
  ...
  private getExecutedFlowNodes(
    flowNodeInstances: DataModels.FlowNodeInstances.FlowNodeInstance[],
    processInstanceState: string,
    flowNodeInstancesTriggeredByThisProcessInstance: DataModels.FlowNodeInstances.FlowNodeInstance[],
  ) {
    const executedFlowNodes: FlowNode[] = [];
    const triggeredFlowNodeInstancesByFlowNodeInstanceId: {
      [flowNodeInstanceId: string]: DataModels.FlowNodeInstances.FlowNodeInstance;
    } = {};
    ...
    for (const flowNodeInstance of flowNodeInstances) {
      ...
      if (!executedFlowNode) {
        ...
        executedFlowNode = new FlowNode(
          ...
          processInstanceState,
          documentation,
          triggeredFlowNodeInstancesByFlowNodeInstanceId[flowNodeInstance.flowNodeInstanceId],
        );
